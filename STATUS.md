...
**Debugging note (corrected).** The earlier "float32 floor" conclusion was wrong. `dL/d(lam)` ~5e-6 at M=8 IS resolvable — the real blocker was a test-side typo (`ce_of` used `std::exp` where `std::log` belonged, bug #5) that halved the apparent CE and masked a constant-factor scale defect for a full turn. The fix: perturb the **cached** `lam` (not a parameter) and re-evaluate the loss in **double**, which isolates the recursion's scale from the float32 forward floor. This surfaced a missing `1/M` (rel error exactly `1/M` at M=2/3 — 0.5/0.67 — a signature noise cannot fake). Two prior "fixes" (cum_prev factor; double-dividing by M) were indeed refuted, but by the batch sweep, not an M=1 reference. **Lesson: when an M=1-only reference agrees with the library, it's not proof — the batch-size sweep is the discriminator, and float32 floor claims need a double-precision loss re-evaluation, not another edit.** Gap #11 is CLOSED.
...
DX12 TESTS: RUN on MSVC 19.52 + Windows SDK 28000 (separate build_dx12/ dir).
             dx12_smoke: 12/12 PASS (device init, weight load, seg0 logits maxdiff
             5.96e-08, halt rows exact, pooled MLP dispatch verified) on AMD RX 9070 XT.
             NOTE: this CPU box has an AMD card and no NVIDIA; DX12 works fine here,
             so do not assume GPU path is untestable — check the actual toolchain.
...
- Gap #10 (GPU backward): FIXED `backward.hlsl` — `total_kept` added to `BwdPushConstants`, `D = d_model` alias defined; all 7 backward entry points compile under dxc. dx12_smoke passes 12/12 on AMD RX 9070 XT (MSVC 19.52 + WinSDK 28000); seg0 logits match golden `maxdiff=5.96e-08`.
